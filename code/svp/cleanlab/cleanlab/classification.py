...
class LearningWithNoisyLabels(BaseEstimator):  # Inherits sklearn classifier
    ...
    def fit(
        ...
        num_to_remove_per_class : :obj: list of int leng K (# classes)
          Remove n samples per class.
        ...
        label_flipping : :obj:bool
          Whether to flip noisy labels rather than prune then.
          Only usable when K=2.
        ...
        if not label_flipping or self.K > 2:
            x_mask = ~self.noise_mask
            x_pruned = X[x_mask]
            s_pruned = s[x_mask]
        else:
            # Flip noisy labels rather than prune then.
            x_pruned = X
            s_pruned = [1 - s[i] if self.noise_mask[i] else s[i] for i in range(len(s))]
        ...
        if hasattr(inspect, 'getfullargspec') and \
            ...
            for k in range(self.K):
                ...
                # Check if error in sample weight, and provide default value
                if math.isnan(sample_weight_k) or math.isinf(sample_weight_k):
                    sample_weight_k = 40
